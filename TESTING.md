...
`com.oop.TestRunner` 클래스에 예제가 들어있다.
...
`com.oop.container.test.Tester` 클래스 안의 `public void runTest` 메소드를 실행하면 `test1~8` 메소드가 실행된다.

`test1~8` 메소드는 순서대로 동시에 실행된다.
...
20개 `Person`을 만들어서 넣는다. 파일 입출력으로 JSON 포맷의 스트링을 저장한다.
